...
while True:
    ...
    for i in range(len(detections)):
        ...
        if conf > float(min_thresh):
            if classname == 'coffee beans':
                ...
                # print(f"✅ Found coffee bean at confidence: {conf:.2f}")

            elif classname == 'coffee beans mold':
                mold_count += 1
                # print(f"⚠️ DANGER: Mold detected at confidence: {conf:.2f}!")
    ...
    # Display detection results
    cv2.putText(frame, f'Number of objects: {object_count}', (10,40), cv2.FONT_HERSHEY_SIMPLEX, .7, (0,255,255), 2) # Draw total number of detected objects
    # Draw specific counts on the frame
    cv2.putText(frame, f'Total Beans: {coffee_beans_count}', (10,40), cv2.FONT_HERSHEY_SIMPLEX, .7, (0,255,255), 2)
    cv2.putText(frame, f'Mold Count: {mold_count}', (10,70), cv2.FONT_HERSHEY_SIMPLEX, .7, (0,0,255), 2) # Highlight mold count in red/blue
    cv2.imshow('YOLO detection results',frame) # Display image
    if record: recorder.write(frame)
...
if source_type == 'video' or source_type == 'usb':
    ...
    cap.stop()
if record: recorder.release()
>>>>>>> 77aa3291751886fcedfc1891e1e8107341118fdb
